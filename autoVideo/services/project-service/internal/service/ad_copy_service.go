...
1. 核心原则：时长优先，少拆而准拆。优先满足当前目标单分镜时长，而不是追求最小视觉单位。
...
5. 广告口播项目中，大多数分镜都应承载明确的 dialogue / 字幕 / 卖点信息；无 dialogue 分镜只能作为极短辅助镜头，不能连续出现，也不能占主要比例。
6. 若同一段文字主要是口播内容，应尽量让口播在一个完整分镜内说完；不要为了动作细节把一句完整口播拆散到多个空镜头中。
