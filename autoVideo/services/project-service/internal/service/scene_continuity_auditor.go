...
7. **空间方位漂移**：同一场景组内人物站位、朝向或关键道具关系无缘由跳变 → 只在 issues 中注明，不要自动往 scene_description 里补“左/右/前景/后景/机位”等构图术语。
8. **空间锚点缺失**：环境结构交代不足 → 只在 issues 中注明；除非 description 完全空白，否则不要补门窗桌椅类空间清单。
9. **位移动作缺失**：人物位置跳变但缺少可见过渡 → 只在 issues 中注明，不要自动补位移动作句。
10. **关键道具连续性丢失**：重要道具无故消失 → 只在 issues 中注明；如必须补充，也只补“仍手持/仍放在桌上”这类动作事实，不写方位坐标。

**修改原则**
- 只补充/更正有明确依据的信息，不凭空发明情节。
- scene_description 必须短、可读、面向创作者：写谁在做什么、什么情绪、环境氛围；禁止写机位、轴线、左/右画幅、前景中景后景分层、镜头衔接套话。
- 若 description 过于单薄，只补 1 句可见动作或表情，不要补空间方位或摄影术语。
- mood / location / characters 有专用字段时，优先补字段，不要把氛围词重复写进 description。
...
func (a *SceneContinuityAuditor) AuditGroup(ctx context.Context, groupKey string, sbs []StoryboardForAudit) ([]ContinuityPatch, error) {
	...
	userContent := fmt.Sprintf(
		"请审查以下场景组（%s）的 %d 帧分镜，检查连贯性并输出修补建议（JSON）。重点检查角色/地点缺失、描述过短、称呼不一致；空间漂移、道具丢失等问题只写入 issues，不要往 scene_description 自动补摄影方位术语。\n\n%s",
