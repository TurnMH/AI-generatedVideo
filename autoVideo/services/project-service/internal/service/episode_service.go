...
- 广告分镜的最高优先级是“先满足当前目标单分镜时长，再决定是否继续拆分”；如果同一段口播/卖点在当前时长内可以完整表达，应优先合并为一个主分镜或少量连续分镜，禁止为了追求最小动作单位而过度拆镜。
...
%s`, episodeNum, episodeNum, content)
		...
		prompt = fmt.Sprintf(`你是一位专业的分镜师和摄影指导。请将以下第 %d 集的内容拆分为适合广告成片的视觉场景（分镜）。

**核心原则：时长优先，少拆而准拆。**
当前目标是优先满足单分镜时长，而不是追求“最小视觉单位”。如果同一段口播、同一段卖点说明、同一段连续动作在当前目标时长内可以完整表达，应优先合并为一个主分镜或少量连续分镜，不要过度拆镜。

**拆分规则（按优先级从高到低）：**
- 第一优先级：先判断当前内容是否可以在目标单分镜时长内完整表达；如果可以，优先保持在同一个分镜内完成
- 只有在以下情况才拆成新分镜：
  1. 明确切换到新空间 / 新场景
...
- 广告口播项目中，大多数分镜都应承载明确的 dialogue / 字幕 / 卖点信息；无 dialogue 分镜只能作为极短辅助镜头，不能连续出现，也不能占主要比例
- 若同一段文字主要是口播内容，应尽量让口播在一个完整分镜内说完；不要为了动作细节把一句完整口播拆散到多个空镜头中
...
- 如果某段只有动作/镜头说明、没有可念文本，dialogue 可以留空；但广告项目中必须尽量减少此类分镜，并确保它只是服务主口播分镜的极短辅助镜头
