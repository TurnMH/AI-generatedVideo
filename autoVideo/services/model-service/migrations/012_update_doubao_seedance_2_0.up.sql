...
-- 注意：历史 seed 可能留下多条相同 model_key 的记录，只更新主记录（最小 id），避免 (name, provider) 唯一约束冲突。
UPDATE models SET
...
UPDATE models SET
  is_active      = false,
  failure_reason = 'duplicate model_key legacy row; superseded by primary doubao-seedance entry',
  updated_at     = NOW()
WHERE model_key = 'doubao-seedance' AND type = 'video'
  AND id <> (
    SELECT id FROM models
