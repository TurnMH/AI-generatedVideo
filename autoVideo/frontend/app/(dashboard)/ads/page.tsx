...
export default function AdsPage() {
  ...
  const adProjects = useMemo(() => projects.filter((project) => matchesProjectMedia(project, 'ad')), [projects])

  const sorted = useMemo(() => {
    const list = [...adProjects]
    list.sort((a, b) => {
      let cmp = 0
      if (sortField === 'title') {
    ...
    return list
  }, [adProjects, sortField, sortDir])

  const projectStats = useMemo(() => {
    const processingCount = adProjects.filter((project) =>
      ['script_processing', 'asset_generating', 'storyboard_generating', 'video_generating'].includes(project.status)
    ).length
    const completedCount = adProjects.filter((project) => project.status === 'completed').length
    const draftCount = adProjects.filter((project) => project.status === 'draft').length
      ...
      <div className="overflow-hidden rounded-[28px] border border-surface-200/70 bg-gradient-to-br from-slate-950 via-violet-950 to-slate-900 p-6 text-white shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="text-2xl font-semibold tracking-tight">{PROJECT_MEDIA_META.ad.listTitle}</h2>
            <p className="mt-2 text-sm leading-6 text-surface-300">
              管理广告脚本、分镜、素材、配音与成片进度，让每个广告项目都沿着统一工作流推进。
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              ...
              onClick={() => router.push(PROJECT_MEDIA_META.ad.createHref)}
            >
              <Plus className="mr-2 h-4 w-4" />
              {PROJECT_MEDIA_META.ad.createLabel}
            </Button>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            ...
            <p className="mt-3 text-2xl font-semibold text-white">{adProjects.length}</p>
            <p className="mt-1 text-xs text-surface-400">{PROJECT_MEDIA_META.ad.countLabel}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            <div className="flex items-center gap-2 text-surface-300">
      ...
      {isLoading ? (
        ...
        <div className="rounded-[28px] border border-surface-200 bg-gradient-to-br from-white to-surface-50 p-3 shadow-sm">
          <div className="rounded-[24px] border border-dashed border-surface-200 bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.08),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(236,72,153,0.08),_transparent_28%)] p-4">
            <EmptyState
              icon={FolderOpen}
              title={keyword || statusFilter !== 'all' ? '没有匹配的广告项目' : PROJECT_MEDIA_META.ad.emptyTitle}
              description={
                keyword || statusFilter !== 'all'
                  ? '尝试调整搜索词或状态筛选，快速找到想继续推进的广告项目。'
                  : PROJECT_MEDIA_META.ad.emptyDescription
              }
              actionLabel={PROJECT_MEDIA_META.ad.createLabel}
              onAction={() => router.push(PROJECT_MEDIA_META.ad.createHref)}
            />
            {!keyword && statusFilter === 'all' ? (
              <div className="grid gap-3 border-t border-surface-200/70 px-4 pb-2 pt-5 sm:grid-cols-3">
                <div className="rounded-2xl bg-white/80 p-4 shadow-sm ring-1 ring-surface-100">
                  ...
                  <p className="mt-1 text-xs leading-5 text-surface-500">先写广告目标、卖点和风格标签，快速建一个广告项目。</p>
                </div>
                <div className="rounded-2xl bg-white/80 p-4 shadow-sm ring-1 ring-surface-100">
                  <p className="text-sm font-medium text-surface-900">串起生产流程</p>
                  <p className="mt-1 text-xs leading-5 text-surface-500">广告脚本、素材、分镜、配音和视频会在项目里持续沉淀。</p>
                </div>
                <div className="rounded-2xl bg-white/80 p-4 shadow-sm ring-1 ring-surface-100">
          ...
          {sorted.map((project) => {
            ...
            return (
              <Card
                ...
                onClick={() => router.push(`/ads/${project.id}`)}
              >
                {/* Cover */}
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenuItem onClick={() => router.push(`/ads/${project.id}`)}>
