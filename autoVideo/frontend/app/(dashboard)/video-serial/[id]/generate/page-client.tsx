...
type SerialVideoLaunchPlan = {
  ...
  totalClips: number
  readySceneGroups: number
  blockedSceneGroups: number
  blockedFirstFrame: number
...
export default function SerialGeneratePage() {
  ...
  const [videoLaunchPlan, setVideoLaunchPlan] = useState<SerialVideoLaunchPlan | null>(null)
  ...
  const buildSerialVideoLaunchPlan = (storyboards: Storyboard[]): SerialVideoLaunchPlan => {
    ...
    let blockedSceneGroups = 0
    let blockedFirstFrame = 0
    let blockedFailed = 0
    let blockedActive = 0
    let readySceneGroups = 0
    let standaloneClips = 0

    const ensureEpisode = (episodeId: number) => {
    ...
    const appendStoryboard = (sb: Storyboard) => {
      ...
      episodeBucket.characters.push(sb.characters || [])
      episodeBucket.assetIds.push(sb.asset_ids || [])
      episodeBucket.sceneGroupKeys.push(sb.scene_group_key || '')
    }

    for (const sb of ordered) {
      if (sb.scene_group_key) {
        const key = `${sb.episode_id ?? 0}:${sb.scene_group_key}`
        const items = serialGroups.get(key) ?? []
        items.push(sb)
        serialGroups.set(key, items)
        continue
      }
      if (sb.status === 'completed' && sb.image_url) {
        appendStoryboard(sb)
        standaloneClips += 1
      }
    }
    ...
    for (const group of serialGroups.values()) {
      ...
      const hasFailed = group.some((sb) => sb.status === 'failed')
      const hasActive = group.some((sb) => ['pending', 'generating', 'paused'].includes(sb.status))
      const hasReadyFirst = firstClip.status === 'completed' && !!firstClip.image_url
      if (!hasReadyFirst || hasFailed || hasActive) {
        blockedSceneGroups += 1
        if (!hasReadyFirst) blockedFirstFrame += 1
        if (hasFailed) blockedFailed += 1
        if (hasActive) blockedActive += 1
        continue
      }
      readySceneGroups += 1
      for (const sb of group) appendStoryboard(sb)
    }
    ...
    const episodes = Array.from(byEpisode.entries())
      ...
      .map(([episodeId, bucket]) => ({
        ...
        scene_group_keys: bucket.sceneGroupKeys.some(Boolean) ? bucket.sceneGroupKeys : undefined,
      }))
    ...
    const fallbackRequest = fallbackBucket && fallbackBucket.imageUrls.length > 0
      ? {
          ...
          serial_scene: true as const,
          scene_group_keys: fallbackBucket.sceneGroupKeys.some(Boolean) ? fallbackBucket.sceneGroupKeys : undefined,
        }
      : undefined
    ...
    return {
      episodes,
      fallbackRequest,
      totalClips: episodes.reduce((sum, item) => sum + item.image_urls.length, 0) + (fallbackRequest?.image_urls.length ?? 0),
      readySceneGroups,
      blockedSceneGroups,
      blockedFirstFrame,
      blockedFailed,
  ...
  const handleStartVideo = async () => {
    try {
      const storyboards = ((await storyboardAPI.listAll(projectId)) as { data?: Storyboard[] }).data ?? []
      const plan = buildSerialVideoLaunchPlan(storyboards)
      if (plan.totalClips === 0) {
        const message = plan.blockedSceneGroups > 0
          ? '当前没有可启动的串行场景组，请先补齐首帧或处理失败分镜。'
          : '暂无可用场景首帧，请先完成首帧准备。'
    ...
    try {
      if (videoLaunchPlan.episodes.length > 0) {
        await videoAPI.generateBatch(projectId, {
          episodes: videoLaunchPlan.episodes,
          serial_scene: true,
        })
      }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {videoLaunchPlan
                ? `本次将启动 ${videoLaunchPlan.totalClips} 个片段，覆盖 ${videoLaunchPlan.episodes.length} 个分集、${videoLaunchPlan.readySceneGroups} 个已就绪场景组。`
                : '将按当前已就绪的分镜状态发起批量视频生成。'}
            </AlertDialogDescription>
            {videoLaunchPlan && videoLaunchPlan.blockedSceneGroups > 0 && (
              <AlertDialogDescription>
                {`另有 ${videoLaunchPlan.blockedSceneGroups} 个场景组会被跳过，以避免首帧缺失或失败分镜导致整批任务中断。首帧未就绪 ${videoLaunchPlan.blockedFirstFrame} 个，含失败分镜 ${videoLaunchPlan.blockedFailed} 个，仍在处理中 ${videoLaunchPlan.blockedActive} 个。`}
              </AlertDialogDescription>
            )}
          </AlertDialogHeader>
          <AlertDialogFooter>
