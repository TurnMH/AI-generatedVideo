...
export default function GeneratePage() {
  ...
  const handleStartVideo = async () => {
    ...
    try {
      ...
      const eligibleSbs = completedSb.filter((sb) => sb.image_url).sort((a, b) => a.sequence_number - b.sequence_number)

      if (eligibleSbs.length === 0) {
      ...
      if (byEpisode.size > 1 || (byEpisode.size === 1 && !byEpisode.has(0))) {
        ...
        if (episodeBatch.length > 0) await videoAPI.generateBatch(projectId, { episodes: episodeBatch })
        const fallbackUrls = byEpisode.get(0)
        if (fallbackUrls?.length) await videoAPI.generate(projectId, { image_urls: fallbackUrls, scene_descriptions: byEpisodeDesc.get(0) })
      } else {
