...
type NewProjectPageClientProps = {
  forcedMediaKind?: ProjectMediaKind
}

export default function NewProjectPageClient({ forcedMediaKind }: NewProjectPageClientProps) {
  const router = useRouter()
  const { toast } = useToast()
  const [mediaKind, setMediaKind] = useState(() => forcedMediaKind ?? normalizeProjectMediaKind(null))
  ...
  useEffect(() => {
    ...
    let nextMediaKind = forcedMediaKind ?? normalizeProjectMediaKind(mediaFromQuery)

    if (draftId) {
      const draft = consumePendingProjectDraft(draftId)
      if (draft) {
        nextMediaKind = forcedMediaKind ?? normalizeProjectMediaKind(mediaFromQuery ?? draft.media ?? null)
        setForm((prev) => {
          const next: FormData = {
            ...prev,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(mediaKind === 'video' ? `${mediaMeta.listHref}/${projectId}${autoStart}` : `${mediaMeta.listHref}?project=${projectId}`)
    } catch {
